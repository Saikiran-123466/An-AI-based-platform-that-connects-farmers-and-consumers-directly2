...
export default function FarmerDashboard() {
  ...
  const fetchOrders = () => {
    axios.get(`http://localhost:5000/api/orders/farmer/${user.id}`)
      .then(res => setOrders(res.data))
      .catch(err => console.log(err));
  };

  const fetchProducts = () => {
    axios.get(`http://localhost:5000/api/products/farmer/${user.id}`)
  ...
  const handleAddProduct = (e) => {
    ...
    if (editingProductId) {
      ...
      // Add new product
      axios.post('http://localhost:5000/api/products', { ...newProduct, farmer_id: user.id })
        .then(() => {
          setShowAddModal(false);
          fetchProducts();
          setNewProduct({ name: '', category: '', price: '', quantity: '', unit: 'kg', organic: false, image: '', cultivated_date: '' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 flex flex-col gap-6">
            <div className="card" style={{ padding: '1.5rem' }}>
              <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderBottom: '1px solid var(--color-border)', paddingBottom: '0.5rem' }}>Your API Listings</h2>
              
              {products.length === 0 ? (
                <p style={{ color: 'var(--color-text-muted)' }}>No products listed yet. Click 'Add Product' to start selling!</p>
              ) : (
                  ...
                  <div key={product.id} className="flex justify-between items-center py-4" style={{ borderBottom: '1px solid #eee' }}>
                    <div className="flex items-center gap-4">
                      <div style={{ width: '60px', height: '60px', borderRadius: 'var(--radius-sm)', background: '#eee', overflow: 'hidden' }}>
                        <img 
                          src={product.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(product.name)}&background=2e7d32&color=fff&size=100`} 
                          alt={product.name} 
                          style={{width:'100%', height:'100%', objectFit:'cover'}}
                        />
                      </div>
                      <div>
                        <h4 style={{ fontWeight: 600 }}>{product.name} {product.organic ? '🌱' : ''}</h4>
                        <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>₹{product.price} / {product.unit} • {product.quantity} {product.unit} in stock</p>
                      </div>
                    </div>
                    <button className="btn btn-outline" style={{ padding: '0.4rem 1rem', fontSize: '0.9rem' }} onClick={() => handleEditClick(product)}>Edit</button>
                  </div>
                ))
              )}
            </div>
            ...
            {/* Recent Orders Area */}
            <div className="card" style={{ padding: '1.5rem' }}>
              <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem', borderBottom: '1px solid var(--color-border)', paddingBottom: '0.5rem' }}>Incoming Orders</h2>
              
              {orders.length === 0 ? (
                <p style={{ color: 'var(--color-text-muted)' }}>No orders have been placed yet.</p>
              ) : (
                  ...
                  {orders.map(order => (
                    <div key={order.id} style={{ border: '1px solid #10b98130', borderRadius: 'var(--radius-md)', padding: '1rem', background: '#f0fdf4' }}>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <p style={{ fontWeight: 600, color: 'var(--color-primary-dark)' }}>{order.product_name} <span style={{fontSize: '0.85rem', color: 'var(--color-text-muted)', fontWeight: 'normal'}}>x {order.quantity}</span></p>
                          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem' }}>Buyer: {order.consumer_name}</p>
                          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem' }}>Ship To: {order.consumer_location}</p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <p style={{ fontWeight: 'bold', color: 'var(--color-secondary)' }}>₹{order.total_price.toFixed(2)}</p>
                          <button className="btn btn-primary" style={{ padding: '0.2rem 0.5rem', fontSize: '0.8rem', marginTop: '0.5rem' }}>Update Status</button>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            <div className="card ai-glow" style={{ padding: '1.5rem', background: '#e8f5e9', border: '2px solid var(--color-primary-light)' }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 style={{ color: 'var(--color-primary-dark)', fontSize: '1.25rem' }}>AI Market Insights</h3>
              </div>
              ...
              {insights ? (
                <div>
                  ...
                  <h4 style={{ marginBottom: '0.5rem', color: 'var(--color-text-main)' }}>Predicted Demand Trends:</h4>
                    ...
                    {insights.demand_trends.map((trend, idx) => (
                      <li key={idx} className="flex justify-between items-center" style={{ background: '#fff', padding: '0.5rem 1rem', borderRadius: 'var(--radius-md)' }}>
                        ...
                        <span className={`badge ${trend.trend === 'High' ? 'badge-green' : 'badge-yellow'}`}>
                          {trend.trend} Demand {trend.predicted_price_increase && `(${trend.predicted_price_increase} ⬆)`}
                        </span>
                      </li>
                    ))}
                  </ul>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', padding: '2rem', borderRadius: '1rem', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className="flex justify-between items-center mb-4">
              <h2 style={{ fontSize: '1.5rem', color: 'var(--color-primary-dark)' }}>{editingProductId ? 'Edit Product' : 'Add New Product'}</h2>
            ...
            <form onSubmit={handleAddProduct} className="flex flex-col gap-4">
              <div className="input-group">
                ...
                <input type="text" className="input" required value={newProduct.name} onChange={e => setNewProduct({...newProduct, name: e.target.value})} placeholder="e.g. Heirloom Tomatoes" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="input-group">
                  <label>Price (₹)</label>
                  <input type="number" className="input" required min="0" step="0.01" value={newProduct.price} onChange={e => setNewProduct({...newProduct, price: parseFloat(e.target.value)})} placeholder="2.50" />
                </div>
                <div className="input-group">
                  <label>Quantity in Stock</label>
                  <input type="number" className="input" required min="1" value={newProduct.quantity} onChange={e => setNewProduct({...newProduct, quantity: parseInt(e.target.value)})} placeholder="100" />
                </div>
              </div>
              ...
              <div className="input-group">
                ...
                <input type="text" className="input" value={newProduct.image} onChange={e => setNewProduct({...newProduct, image: e.target.value})} placeholder="https://..." />
              </div>

              <button type="submit" className="btn btn-primary" style={{ marginTop: '1rem' }}>{editingProductId ? 'Save Changes' : 'Publish Product Listing'}</button>
            </form>
          </div>
        </div>
      )}
